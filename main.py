...
def make_parser():
    ...
    parser.add_argument(
        "--neofile",
        default=(DATA_ROOT / "neos.csv"),
        type=pathlib.Path,
        help="Path to CSV file of near-Earth objects.",
    )
    parser.add_argument(
        "--cadfile",
        default=(DATA_ROOT / "cad.json"),
        type=pathlib.Path,
...
class NEOShell(cmd.Cmd):
    ...
    def do_inspect(self, arg):
        ...
        inspect(self.db, pdes=args.pdes, name=args.name, verbose=args.verbose)
    ...
    def precmd(self, line):
        ...
        changed = [
            f for f in PROJECT_ROOT.glob("*.py") if f.stat().st_mtime > _START
        ]
        if changed:
            print(
...
def main():
    ...
    database = NEODatabase(
        load_neos(args.neofile), load_approaches(args.cadfile)
    )
    ...
    if args.cmd == "inspect":
        ...
        NEOShell(
            database, inspect_parser, query_parser, aggressive=args.aggressive
        ).cmdloop()
